...
手持钉砖右键空气或方块进入固定状态，再次右键进入调整模式。固定状态会关闭重力，并阻止击退、活塞和外部传送造成的位置变化；只有按下移动键才会解除固定，转动视角不会解除。调整模式下按住普通移动键会以 `0.05` 格/刻微调，按住冲刺键会以 `0.5` 格/刻大调；W/S 沿面向方向前后调整，A/D 左右调整，跳跃上移，按 Shift 下移。调整过程中外部位移仍会被拦截，完成后左键即可回到固定状态。
