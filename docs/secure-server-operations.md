...
- 最大人数：12 人。
- 服务端口：TCP `22851`（公网地址 `play.simpfun.cn:22851`）。
...
- 视距 6、模拟距离 4，适合小型服务器。
- 当前公网实例运行在简幻欢云端，使用其分配的单端口地址；本地 Playit 隧道已停止。

## 启动与停止

云端实例在简幻欢控制台的“终端”页启动和停止。连接地址为 `play.simpfun.cn:22851`。
